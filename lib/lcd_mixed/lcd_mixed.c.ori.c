...
{
	//setup lcd pin
	bit_set(LCD_RS_DDR, BIT(LCD_RS_BIT));
	bit_clear(LCD_RS_PORT, BIT(LCD_RS_BIT));
	
	#if (USE_BUSY_FLAG == 1)
	bit_set(LCD_RW_DDR, BIT(LCD_RW_BIT));
	bit_clear(LCD_RW_PORT, BIT(LCD_RW_BIT));
	#endif
	
	bit_set(LCD_EN_DDR, BIT(LCD_EN_BIT));
	bit_clear(LCD_EN_PORT, BIT(LCD_EN_BIT));
	
	//control pin lcd
	bit_set(LCD_D4_DDR, BIT(LCD_D4_BIT));
	bit_clear(LCD_D4_PORT, BIT(LCD_D4_BIT));
	bit_set(LCD_D5_DDR, BIT(LCD_D5_BIT));
	bit_clear(LCD_D5_PORT, BIT(LCD_D5_BIT));
	bit_set(LCD_D6_DDR, BIT(LCD_D6_BIT));
	bit_clear(LCD_D6_PORT, BIT(LCD_D6_BIT));
	bit_set(LCD_D7_DDR, BIT(LCD_D7_BIT));
	bit_clear(LCD_D7_PORT, BIT(LCD_D7_BIT));
}
